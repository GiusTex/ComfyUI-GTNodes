...
from .utils import (pil2tensor, global_preview_method, warning, 
                   ...
                   globals_cleanup, sample_custom_ultra, sample_custom_ultra_plus)
...
class SamplerCustomUltraAdvancedPlusEfficient:
    # Image Preview code taken from jags111's efficiency-nodes (TSC_KSampler)
    empty_image = pil2tensor(Image.new('RGBA', (1, 1), (0, 0, 0, 0)))
    ...
    RETURN_TYPES = ("MODEL", "SAMPLER", "SIGMAS", "LATENT","LATENT", "IMAGE", "VAE",)
    RETURN_NAMES = ("model", "sampler", "sigmas", "output", "denoised_output", "image", "vae", )
    FUNCTION = "sample"
    CATEGORY = "More Efficient Samplers"

    #def sample(self, model, add_noise, noise_seed, cfg, positive, negative, sampler, sigmas, latent_image, start_at_step, end_at_step, return_with_leftover_noise, preview_method, vae_decode, optional_vae=(None,), prompt=None, extra_pnginfo=None, my_unique_id=None):
    def sample(self, model, add_noise, noise_seed, guider, sampler, sigmas, latent_image, start_at_step, end_at_step, return_with_leftover_noise, preview_method, vae_decode, optional_vae=(None,), prompt=None, extra_pnginfo=None, my_unique_id=None):
        latent = latent_image
        latent_image = latent["samples"]
        latent = latent.copy()
        latent_image = comfy.sample.fix_empty_latent_channels(model, latent_image)
        ...
        def process_latents():
            ...
            try:
                ...
                samples = sample_custom_ultra_plus(noise, sampler, guider, latent_image, start_at_step, end_at_step, force_full_denoise, noise_mask, sigmas, callback, disable_pbar, noise_seed)
                samples = samples.to(comfy.model_management.intermediate_device())
                ...
                # Define a dummy output image
                if images is None and vae_decode == "false":
                    images = SamplerCustomUltraAdvancedPlusEfficient.empty_image
